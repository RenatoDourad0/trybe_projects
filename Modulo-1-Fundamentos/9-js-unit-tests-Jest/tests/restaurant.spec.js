...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    expect(Object.entries(returnedObj.fetchMenu())).toEqual(Object.entries(exampleMenu));
    ...
    expect(returnedObj.consumption).toEqual(['coxinha', 'agua']);
    // TESTE 7: Verifique se a função `order` aceita que pedidos repetidos sejam acrescidos a `consumption`.
    returnedObj.order('coxinha');
    returnedObj.order('agua');
    returnedObj.order('coxinha');
    expect(returnedObj.consumption).toEqual(['coxinha', 'agua', 'coxinha', 'agua', 'coxinha']);
    ...
    expect(returnedObj.pay()).toBe(19.5);
  });
});
